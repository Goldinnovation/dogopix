...
import { SignUpUserAPI } from "../../api/signUp/signUpApi";
...
const SignUpComponentStructure = () => {
  ...
  const handleSignUpDataWithoutValidation = async () => {
    const userData = {
      userEmail: email,
    ...
    const signUpReq = await SignUpUserAPI(userData);


    if (signUpReq.message == "created new Account") {
      setApprovalMessage(true);
      setTimeout(() => {
        setApprovalMessage(false);
        navigation.goBack();
  ...
  const handleInputData = () => {
    ...
    handleSignUpDataWithoutValidation();
  };
